...
- **`src/tiktok/photoPost.ts` + `ffmpeg.ts`** — у фото-постов TikTok (`tiktok.com/@user/photo/...`) вообще нет видео, только слайд-шоу картинок с музыкальной дорожкой, и встроенного JSON у них тоже нет — эти данные приходят через клиентский XHR, который делает сама страница TikTok, и `photoPost.ts` его дожидается. Затем он параллельно скачивает каждую картинку и аудиодорожку через тот же приём с загрузкой через Chromium, что и для видео (с одной повторной попыткой при кратковременном сбое), а `ffmpeg.ts` вызывает **ffmpeg** (и `ffprobe` — определить кодек аудио и пропустить перекодирование, если это уже AAC), чтобы собрать их в настоящий mp4 — каждая картинка показывается не меньше 3 секунд, даже если из-за этого слайд-шоу выходит длиннее самой музыки (тогда аудио просто зацикливается, чтобы заполнить разницу). Жёсткий таймаут на процесс ffmpeg защищает от зависания кодирования навсегда. Дальше по пайплайну (кэш, загрузка в Telegram) это обычное видео.
- **`src/tiktok/timing.ts`** — необязательные, ничего не делающие по умолчанию логи тайминга (`DEBUG_TIMING=true`) для диагностики медленного или зависшего парсинга: переход на страницу, скачивание картинок/аудио и кодирование ffmpeg — каждый шаг пишет строку `[timing]` с длительностью в мс.
...
| `DEBUG_TIMING` | необязательна | `true` — включить подробные логи тайминга (переход на страницу, скачивания, кодирование ffmpeg) для диагностики медленного парсинга; по умолчанию `false` |
...
Установите `DEBUG_TIMING=true` в `.env`, чтобы получить подробные логи по этапам (переход на страницу, скачивание картинок/аудио, кодирование ffmpeg) при разборе медленного или зависшего парсинга.
